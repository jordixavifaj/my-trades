...
export function ReportsDashboard() {
  ...
  const loadReports = useCallback(async () => {
    ...
    try {
      ...
      if (setup.trim()) qs.set('setup', setup.trim());
      if (ecnFeePerShare.trim()) qs.set('ecnFeePerShare', ecnFeePerShare.trim());
      const res = await fetch(`/api/dashboard/reports?${qs.toString()}`, { cache: 'no-store' });
      const json = await res.json();
      if (!res.ok) {
        setError(json.error || json.detail || 'No se pudo cargar Reports');
        setData(null);
        return;
      }
      setData(json);
  ...
  }, [from, to, ticker, setup, ecnFeePerShare]);
    ...
    <div className="space-y-6">
      <section className="panel p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <form
            className="grid w-full gap-2 sm:grid-cols-2 lg:w-auto lg:grid-cols-5"
            onSubmit={(e) => {
              e.preventDefault();
              void loadReports();
            }}
          >
            ...
            <label className="text-xs text-slate-400">
              ...
              >
                ...
                <option value="UNLABELED">Sin setup</option>
                {setups.map((s) => (
            ...
            <label className="text-xs text-slate-400">
              ...
              <input
                className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900/80 p-2 text-sm"
                value={ecnFeePerShare}
                onChange={(e) => setEcnFeePerShare(e.target.value)}
                placeholder="0.003"
              />
            </label>
      ...
      {data && (
        <>
          {/* ---- Tradervue-style Statistics Panel ---- */}
          <section className="panel p-5">
          ...
          <section className="grid gap-4 md:grid-cols-3">
            ...
            <article className="panel p-4">
              ...
              <p className="mt-1 text-xs text-slate-500">
                Gross: {data.totals.pnlGrossTotal.toFixed(2)} · ECN: {data.totals.ecnFeesTotal.toFixed(2)}
              </p>
            </article>
            <article className="panel p-4">
